/* Numbers At Most N Given Digit Set */
/** Given an array of digits, you can write numbers using each digits[i] as many times as we want.  
 *  For example, if digits = ['1','3','5'], we may write numbers such as '13', '551', and '1351315'.
 *  Return the number of positive integers that can be generated that are less than or equal to a given integer n.
 *  
 *  Example 1:
 *  Input: digits = ["1","3","5","7"], n = 100
 *  Output: 20
 *  Explanation: 
 *  The 20 numbers that can be written are:
 *  1, 3, 5, 7, 11, 13, 15, 17, 31, 33, 35, 37, 51, 53, 55, 57, 71, 73, 75, 77.
 * 
 *  Example 2:
 *  Input: digits = ["1","4","9"], n = 1000000000
 *  Output: 29523
 *  Explanation: 
 *  We can write 3 one digit numbers, 9 two digit numbers, 27 three digit numbers,
 *  81 four digit numbers, 243 five digit numbers, 729 six digit numbers,
 *  2187 seven digit numbers, 6561 eight digit numbers, and 19683 nine digit numbers.
 *  In total, this is 29523 integers that can be written using the digits array.
 * 
 *  Example 3:
 *  Input: digits = ["7"], n = 8
 *  Output: 1
 *  
 *  Constraints:
 *  1 <= digits.length <= 9
 *  digits[i].length == 1
 *  digits[i] is a digit from '1' to '9'.
 *  All the values in digits are unique.
 *  1 <= n <= 10^9  **/

/* 아래는 leetcode 사이트에서 솔루션을 보고 구현한 풀이 */

#include <vector>
#include <cstring>
#include <string>
#include <iostream>
#include <math.h>
using namespace std;

int atMostNGivenDigitSet(vector<string>& digits, int n) {
    string n_str = to_string(n);
    int length = n_str.length();
    int cache[length+1];
    int answer = 0;

    /* cache[i]는 n_str[length-i], n_str[length-i+1], ... n_str[length-1] 로 이루어진 수 이하면서
       digits의 원소들로 만들어질 수 있는 i 자리 수의 개수이다 */
    
    memset (cache, 0, sizeof(cache));
    cache[0] = 1;

    // 아래는 valid한 수 중 자리수가 length인 수들의 개수를 구하는 과정
    for (int i = 1; i <= length; ++i) {
        int s = n_str[length-i] - '0';
        for (string iter : digits) {
            if (stoi(iter) < s) {
                cache[i] += pow(digits.size(), i-1);
            } else if (stoi(iter) == s) {
                cache[i] += cache[i-1];
            }
        }
    }

    answer = cache[length];

    // 아래는 valid한 수 중 자리수가 length 이하인 수들의 개수를 더하는 과정
    for (int i = 1; i <= length-1; ++i) {
        answer += pow(digits.size(), i);
    }

    return answer;
}

int main() {
    int n = 64;
    //vector<string> digits {"1","3","5","7"};
    //vector<string> digits {"1","4","9"};
    //vector<string> digits {"7"};
    //vector<string> digits {"9"};
    vector<string> digits {"3","4","5","6"};

    int answer = atMostNGivenDigitSet(digits, n);
    cout << answer << endl;

    return 0;
}