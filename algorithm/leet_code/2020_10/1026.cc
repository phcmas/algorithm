...
double champagneTower(int poured, int query_row, int query_glass) {
    double champagne[100][100];
    champagne[0][0] = (double) poured;

    for (int i = 0; i <= query_row; ++i) {
        for (int j = 0; j <= query_glass; ++j) {
            double distributed = champagne[i][j]-1.0;
            if (distributed > 0) {
                champagne[i][j] = 1;
                champagne[i+1][j] += distributed/2.0;
                champagne[i+1][j+1] += distributed/2.0;
            }
        }
    }
...
int main() {
    double answer = champagneTower(25,6,1);
    cout << answer << endl;
