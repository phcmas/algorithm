...
class Solution:
    def predict_the_winner(self, nums: List[int]) -> bool:
        length = len(nums)
        player1 = [[0 for i in range(length)] for j in range(length)]
        player2 = [[0 for i in range(length)] for j in range(length)]
        ...
        for i in range(1, length):
            ...
            for j in range(i - 1, -1, -1):
                sum += nums[j]
                player1[j][i] = max(
                    nums[j] + player2[j + 1][i], nums[i] + player2[j][i - 1]
                )
                player2[j][i] = sum - player1[j][i]

        return player1[0][length - 1] >= player2[0][length - 1]
